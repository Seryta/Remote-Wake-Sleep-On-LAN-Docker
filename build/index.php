    	...
    	<form class="form-signin" method="post">
            ...
            <?php
                ...
                elseif ($wake_up)
                {
					...
					// Send the magic packet as a Layer-2 frame with etherwake instead of the
					// UDP-broadcast based wakeonlan: some NIC drivers (e.g. Intel I225-V)
					// only accept L2 frames addressed to the target MAC.
					// If $WOL_INTERFACE is unset, pick the interface of the default route.
					$wol_iface = !empty($WOL_INTERFACE)
						? $WOL_INTERFACE
						: trim(shell_exec("ip route get 8.8.8.8 2>/dev/null | awk '{print \\$5}'"));
					$wol_cmd = 'etherwake' . ($wol_iface !== '' ? ' -i ' . escapeshellarg($wol_iface) : '') . ' ' . $COMPUTER_MAC[$selectedComputer];
					exec ($wol_cmd);
